    ...
    webSocket.onopen = function(event) {
        ...
        webSocket.send(JSON.stringify({"type" : getCookie("type"), "name" : getCookie("room_name"), "equipment" : equipment}));
    };
        ...
        if (CURRENT_STATE != FINAL__somebody_win) {
            ...
            setTimeout('location="http://' + ipAddr + ':' + portHttp +'"', 3000);
                ...
                create_button.onclick = function() {
                    setCookie("type", "create", 1);
                    setCookie("room_name", document.getElementById("room_name").value, 1);
                    setCookie("equipment", "0_0_0", 1);
                    if (! webSocketOpen) {
                        webSocket = new WebSocket('ws://' + ipAddr + ':' + portWebsockets);
                        webSocketOpen = true;
                        /* Действия, выполняемые в начале общения сервера и клиента */
