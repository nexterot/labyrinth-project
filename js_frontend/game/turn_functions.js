...
function drawGo(data) {
    if (data.error == 1) {
        ...
        if (data.is_here_enemy == 1)
            drawAnotherHelper(data, data.from_coordinates[0], data.from_coordinates[1], data.from_sprite, "player_ghost", data.name); // Пройтись по массиву имен (нет)
        if (data.is_here_enemy == 2)
            drawAnotherHelper(data, data.from_coordinates[0], data.from_coordinates[1], data.from_sprite, "player_stay", data.name); // Пройтись по массиву имен (нет)
